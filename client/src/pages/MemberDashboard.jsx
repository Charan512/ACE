...
const MemberDashboard = () => {
  ...
  useEffect(() => {
    // Stub fetch - In Phase 7 we will integrate `api.get('/events')` and User's `history.attendedEvents`
    const mockEvents = [
      {
        _id: '1',
        title: 'CyberSec 2026 Hackathon',
        description: 'A 24-hour intensive CTF and building session.',
    ...
    const mockVault = [
      {
        ...
        eventDate: '2025-10-10T10:00:00.000Z',
        certificateUrl: '/api/certificates/99' // mock endpoint
      }
    ];

    setTimeout(() => {
      setUpcomingEvents(mockEvents);
      setVaultEvents(mockVault);
      setLoading(false);
    }, 500);
  }, []);
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {loading ? (
              <p className="font-data text-text-disabled">Loading events...</p>
            ) : (
        ...
        <section>
          ...
          {loading ? (
             <p className="font-data text-text-disabled">Decrypting vault...</p>
          ) : vaultEvents.length === 0 ? (
             <p className="font-data text-text-disabled">No records found.</p>
          ) : (
