...
graph TD
    ...
    CC --> |"Fetch Templates"| Cloudinary
    PaymentC <--> |"S2S Webhooks & Validation"| PhonePe
    EC --> |"Transport Emails"| Brevo
```
...
| **Email Delivery** | Brevo | Transactional HTTPS API — guarantees clean and unbranded delivery. |
| **Certificate Engine** | `@napi-rs/canvas` | Node canvas engine fetching assets to RAM (zero local storage). |
| **Image Storage** | Cloudinary | Asset storage for user avatars and certificate templates via streaming. |
...
* `CLOUDINARY_URL`: Full connection string (optional, but generated by Cloudinary).

### Email Delivery (Brevo)
The backend uses Brevo's Transactional HTTPS API (via `@getbrevo/brevo`). This guarantees clean, unbranded delivery and completely bypasses Render's SMTP port blocks.
* `BREVO_API_KEY`: Your Brevo API key (starts with `xkeysib-`). Get one free at [brevo.com](https://brevo.com).
* `EMAIL_FROM`: The sender address shown to recipients (must be verified in Brevo).
...
* A [Brevo](https://brevo.com) account with a verified sending domain and API key.
